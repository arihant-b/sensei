...
from sensei.data.bins import EncodedSample  # noqa: E402
...
def test_le_rule() -> None:
    ...
    assert DomainRuleChecker.check_rule(EncodedSample({"exp": 20, "age": 40}), rule)
    assert not DomainRuleChecker.check_rule(EncodedSample({"exp": 25, "age": 40}), rule)


def test_eq_rule_with_tolerance() -> None:
    rule = DomainRule(coefficients={"a": 1.0, "b": -1.0}, sense="==", rhs=0.0)

    close = EncodedSample({"a": 5.0, "b": 5.0000000001})
    far = EncodedSample({"a": 5.0, "b": 5.1})
    assert DomainRuleChecker.check_rule(close, rule)
    assert not DomainRuleChecker.check_rule(far, rule)


def test_rule_skipped_when_feature_missing():
    rule = DomainRule(coefficients={"nonexistent": 1.0}, sense="<=", rhs=0.0)
    assert DomainRuleChecker.check_rule(EncodedSample({"a": 1.0}), rule)
